...
from .ham import *            #  Hybrid Attention Module
...
class Dblock(nn.Module):
    ...
    def forward(self, x):
        dilate1_out = nonlinearity(self.dilate1(x))
        # dilate1_out = self.ham1(dilate1_out)
...
class UGDDLinkNet(nn.Module):
    def __init__(self, num_classes=2, drop_rate = 0.2):     
        super(UGDDLinkNet, self).__init__()
        ...
        self.ham1 = ham(in_channel=filters[0], ratio=16)
        self.ham2 = ham(in_channel=filters[1], ratio=16)
        self.ham3 = ham(in_channel=filters[2], ratio=16)

        self.drop1 = nn.Dropout(p = drop_rate)
        self.dblock = Dblock(512)  # DCAM
