...
class dens_block(nn.Module):
    ...
    def forward(self,input_tensor):
        ...
        x2 = self.conv2(torch.cat([x1,input_tensor],dim=1))
        # add2 =torch.cat([x1, input_tensor,x2], dim=1)
        x3 = self.conv3(torch.cat([x1, input_tensor,x2], dim=1))
        return x3
...
class DenseU_Net(nn.Module):
    def __init__(self, img_ch=3, output_ch=1,drop_rate=0.4):
