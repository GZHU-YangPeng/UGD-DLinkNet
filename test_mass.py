...
from train_mass import CLASSES ,PALETTE
from model import DeepLabV3Plus
from utils.data_manager import DeepGlobe,MassaChu
import matplotlib.pyplot as plt
from model.models import U_Net,AttU_Net
from model.resnet50_unet import *
# from model.Dlinknet import *
from model.UGD_Dlinknet import *
from model.UGD_cbam import *

from model.models import U_Net,AttU_Net,SegNet
from model.resnet50_unet import *
# from model.models import U_Net
...
def main():
    ...
    parser.add_argument('--model-file', type=str, default="../logs/MassChu/ts_t_drop_cls/Dlinknet_Hybrid/S_20250711_014851.213641_False/best.pth.tar",
    ...
    parser.add_argument(
        '--save-dir',
        type=str,
        default='../logs/MassChu/ts_t_drop_cls/Dlinknet_Hybrid/S_20250711_014851.213641_False/results',
...
# 1. dataset
    dataset = MassaChu(root = args.datasetdir)
...
#数据增强
    test_transforms = Compose(
         [
            LoadImaged(keys=["img","label"], reader=PILReader, dtype=np.float32, meta_key_postfix='meta_dict'),
             # LoadImaged(keys=[ "label"], reader=PILReader, dtype=np.float32),
...
# 2. model
    ...
    # model = U_Net(in_ch=3 , out_ch= args.num_classes, drop_rate=args.dropout_rate)
    # model=DeepLabV3Plus(classes=args.num_classes,dropout_rate=args.dropout_rate)
    # model = U_Net(in_ch=3 , out_ch= args.num_classes, drop_rate=args.dropout_rate)
    model = UGDDLinkNet(num_classes= args.num_classes, drop_rate=args.dropout_rate)  #UGD
    
#     model = DinkNet34(num_classes= args.num_classes)   # 
    # model = models.segmentation.fcn_resnet50(num_classes = 2, pretrained=False ,drop_rate=args.dropout_rate)
    # model = SegNet(num_classes=args.num_classes, n_init_features=3, drop_rate=args.dropout_rate)
        ...
        for batch_idx, (sample) in tqdm.tqdm(enumerate(test_loader),
            ...
            if args.mc_samples > 1:
                ...
                for i in range(args.mc_samples):
                    ...
                    # prediction = prediction['out']   # FCN-Resnet50 专用
                ...
                # plt.subplot(2,4,1)
                # plt.imshow(entropy[0].cpu().numpy())
                # plt.subplot(2,4,2)
                # plt.imshow(label[0].cpu().numpy())
                # plt.subplot(2,4,3)
                # plt.imshow(pesudo_1[0].cpu().numpy())
                # plt.subplot(2,4,4)
                # plt.imshow(pesudo_2[0].cpu().numpy())
                # # visualize the distribution of entropy
                # plt.subplot(2,4,5)
                # plt.hist(entropy[0].cpu().numpy().reshape(-1),bins=5,range=(0.1,1))
                # plt.subplot(2,4,6)
                # plt.hist(label[0].cpu().numpy().reshape(-1),bins=5)
                # plt.subplot(2,4,7)
                # plt.hist(pesudo_1[0].cpu().numpy().reshape(-1),bins=5,range=(0.1,1))
                # plt.subplot(2,4,8)
                # plt.hist(pesudo_2[0].cpu().numpy().reshape(-1),bins=5,range=(0.1,1))
                # plt.show()
