...
from model.models import AttU_Net,R2U_Net,U_Net
...
from model.UGD_cbam import *     #UGD_Dlinknet  with cbam
# import datetime
...
# '../logs/MassChu/ts_t_drop_cls/DLinkNet34/S_20240317_235948.081965_False/best.pth.tar'
parser.add_argument(
...
parser.add_argument(
        '--num-workers',type=int,default=12,help='how many subprocesses to use for dataloading.' # 默认12 设置为4
)
parser.add_argument(
        '--input-size',type=int,default=512,help='input image size'
    )
parser.add_argument(
        '--dropout-rate',type=float,default=0.5,help='mc dropout rate'   # 默认0.5 考虑 设置0.1-0.5
    )
parser.add_argument(
...
parser.add_argument(
        '--max-epoch', type=int, default=100, help='max epoch'    #默认100  可根据学习率调整
    )
parser.add_argument(
        '--stop-epoch', type=int, default=100, help='stop epoch'  #默认100  可根据学习率调整
    )
parser.add_argument( 
        '--interval-validate', type=int, default=1, help='interval epoch number to valide the model'
    )
parser.add_argument(
        '--lr-model', type=float, default=1e-3, help='learning rate'    # 默认1e-3   可以设置 1e-4  epoch 200+
    )
parser.add_argument(
        '--seed',type=int,default=3407,help='set random seed'
    )
parser.add_argument(
        '--model-name',type=str,default='UGDDLinkNet',help='network name. \
...
parser.add_argument('--teach', type=bool,
                    default=True, help='train teacher and student together?')    # 默认 true 可以考虑设为 False
...
#     t_model=DeepLabV3Plus(encoder_weights="imagenet",classes=args.num_classes,dropout_rate=args.dropout_rate)
    #UGD-DLinkNet
    t_model = UGDDLinkNet(num_classes= args.num_classes, drop_rate=args.dropout_rate)    #UGD-DLinkeNet
    
#     t_model = DinkNet34(num_classes= args.num_classes)   # DLinkeNet-CBAM
...
#     t_model = UNetWithResnet50Encoder(n_classes=args.num_classes,drop_rate= args.dropout_rate)
    ...
    # s_model = U_Net(in_ch=3 , out_ch= args.num_classes, drop_rate=0.2)
    print("train s_model useing DLinkNet34")
    
    #UGD-DLinkNet
    s_model = UGDDLinkNet(num_classes= args.num_classes, drop_rate=args.dropout_rate)   #UGD-DLinkeNet

#     s_model = DinkNet34(num_classes= args.num_classes)   # DLinkeNet-CBAM  
    # s_model = AttU_Net(img_ch=3 , output_ch= args.num_classes, drop_rate=0.2)
