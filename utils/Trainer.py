        ...
        with torch.no_grad():
            for batch_idx, sample in tqdm.tqdm(
                ...
                with torch.no_grad():
                    ...
                    # predictions  = self.s_model(data)  #  FCN-resnet50专用
                    # predictions = predictions['out']  #  FCN-resnet50专用
        ...
        for batch_idx, sampleS in tqdm.tqdm(
            ...
            if self.together:
                with torch.no_grad():
                    ...
                    # outputT  = self.t_model(images)    #  FCN-resnet50专用
                    

            # t_output = torch.softmax
            # outputS,s_e2,s_d3,outputS_dis,s_e5 = self.s_model(images)
            outputS,e4_s= self.s_model(images)
            # outputS = self.s_model(images)    #  FCN-resnet50专用
            # outputS = outputS['out']            #  FCN-resnet50专用
            ...
            if self.together:
                with torch.no_grad():
                    ...
                    for i in range(self.mc_samples):
                        ...
                        # t_output = self.t_model(images)              #  FCN-resnet50专用
                        # t_output=t_output['out']                #  FCN-resnet50专用
                        # outputT_soft = torch.softmax(t_output, dim=1)  #  FCN-resnet50专用
                ...
                #学生模型输出和教师模型输出之间作损失
                consistency_dist = 1 * self.consistency_criterion(outputS, outputT) + 2 * self.consistency_criterion(e4_s,e4_t)
                # print(consistency_dist.shape,mask.shape)
                # consistency_dist = 1 * kl_loss(F.log_softmax(outputS,dim=1),F.log_softmax(outputT,dim=1)) + 1 * kl_loss(F.log_softmax(e4_s,dim=1),F.log_softmax(e4_t,dim=1)) # 换成这个
            ...
            # print(loss_ce,consistency_loss,loss_de,consistency_kd1,consistency_kd2,consistency_weight)
                loss = loss_ce + loss_de * self.lambda_soft + consistency_loss * 1e2  #+ loss_kd * self.lambda_kd * 1e1 * 5
            else:
                loss = loss_ce + loss_de * self.lambda_soft
