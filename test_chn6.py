...
from utils.data_manager import DeepGlobe,MassaChu,CHN6_test
...
# 1. dataset
    dataset = CHN6_test(root = args.datasetdir)
...
#     model = U_Net(in_ch=3 , out_ch= args.num_classes, drop_rate=args.dropout_rate)
    # model = UGDDLinkNet(num_classes= args.num_classes, drop_rate=args.dropout_rate)
#     model = DinkNet34(num_classes= args.num_classes)   # DLinkeNet-CBAM
        ...
        for batch_idx, (sample) in tqdm.tqdm(enumerate(test_loader),
            ...
            if args.mc_samples > 1:
                ...
                for i in range(args.mc_samples):
                    ...
                    prediction, _ = model(data)  # 忽略第二个返回值  UGD-DLinkNet
