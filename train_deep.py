...
from model.UGD import *
from model.UGD_cbam import *
# import datetime

# from model.Dlinknet_Hybrid import *   
# from model.Dlinknet_CBAM import * 
...
parser.add_argument(
        '--max-epoch', type=int, default=110, help='max epoch'
    )
parser.add_argument(
        '--stop-epoch', type=int, default=110, help='stop epoch'
    )
parser.add_argument(
...
parser.add_argument('--consistency', type=float,
 ...
 # 设置False:关闭教师模型    True:打开
parser.add_argument('--teach', type=bool,
                    default=True, help='train teacher and student together?')
...
#     t_model=DeepLabV3Plus(encoder_weights="imagenet",classes=args.num_classes,dropout_rate=args.dropout_rate)
    # t_model = DinkNet34(num_classes= args.num_classes, drop_rate=args.dropout_rate)
    t_model = UGDDLinkNet(num_classes= args.num_classes, drop_rate=args.dropout_rate) 
...
#     t_model = UNetWithResnet50Encoder(n_classes=args.num_classes,drop_rate= args.dropout_rate)
    ...
    # s_model = DinkNet34(num_classes= args.num_classes, drop_rate=args.dropout_rate)
    #UGD-DLinkNet
    s_model = UGDDLinkNet(num_classes= args.num_classes, drop_rate=args.dropout_rate)
